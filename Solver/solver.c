...
int isSafe(long grid[N][N], long row,
                       long col, long num)
{
   ...
   if ( fp )
      {
		   ...
		   if (row == 9){ // if we reach end of row, new column
			   row = 0;
			   col++;
		   }

		   if (isdigit(ch)){ // check if number (1-9)
			   grid[col][row]= ch - '0';
		           row++;
		   }

           else if(ch == '.'){ // check if empty spot
			   grid[col][row]=0;
			   row++;
		   }
         ...
         if (solveSudoku(grid, 0, 0)){ // if the sudoku has been solved
             print(grid);} // We write it into the result file
         else {
